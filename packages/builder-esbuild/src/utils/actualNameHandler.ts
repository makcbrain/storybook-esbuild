 ...
 */
import type { babelTypes, Handler, NodePath } from 'react-docgen';
import { utils } from 'react-docgen';

const { getNameOrValue, isReactForwardRefCall } = utils;

export const actualNameHandler: Handler = function actualNameHandler(
	documentation,
	componentDefinition,
) {
	...
	if (
		...
		documentation.set(
			'actualName',
			getNameOrValue(componentDefinition.get('id') as NodePath<babelTypes.Identifier>),
		);
	} else if (
